...
#define DEBUG
#undef DEBUG // Used to remove the extra messages when printing to LCD
#undef ASM_CODE
...
void showSeq(int *seq)
{
	for (int i = 0; i < seqlen; i++)
	{
		char color = color_names[seq[i] - 1][0];
...
{
	int exactMatches = 0, approxMatches = 0;

	// Create copies
	memcpy(cpy1, seq1, sizeof(int) * SEQL);
	memcpy(cpy2, seq2, sizeof(int) * SEQL);

	int* mres = matches(cpy1, cpy2);
	return mres;
	...
	// result[0] = exactMatches;
	// result[1] = approxMatches;
...
{
	// Keep using modulo and division to get the digits
...
void initITimer(uint64_t timeout)
{
	...
	// Install the timer_handler callback
...
void printLCD(struct lcdDataStruct *lcd, char *str1, char *str2)
{
	lcdClear(lcd);
	lcdPosition(lcd, 0, 0); lcdPuts(lcd, str1);
...
int main(int argc, char *argv[])
{
	...
	int pinGreen = LED, pinRed = LED2, pinButton = BUTTON;
	...
	pinMode(gpio, pinGreen, OUTPUT); // Config Yellow LED (GPIO 13) as output
	writeLED(gpio, pinGreen, LOW);

	pinMode(gpio, pinRed, OUTPUT); // Config Red LED (GPIO 5) as output
	writeLED(gpio, pinRed, LOW);

	pinMode(gpio, pinButton, INPUT); // Config Button (GPIO 19) as input
	...
	printLCD(lcd, "Welcome to", "Mastermind");
	delay(2000);

	if (debug)
	{
		printLCD(lcd, "Click Enter", "to Continue");
		waitForEnter();
	}
	else
	{
		printLCD(lcd, "Click Button", "to Continue");
	...
	char round[17], elem[17], exact[17], approx[17];
	int exactNum = 0, approxNum = 0;
	...
	while (!found && attempts < MAX_ATTEMPTS)
	{
		...
		blinkN(gpio, pinRed, 3);
		// delay(DELAY_LONG);

		sprintf(round, "Round:%d  E:%d A:%d", attempts, exactNum, approxNum);

		if (debug)
		{
			printf("Round %d\nEnter Sequence: ", attempts);
			printLCD(lcd, round, "Enter Sequence");
			int num = readNum(seqlen);
			readSeq(attSeq, num);
		}
		else
		{
			...
			for (int i = 0; i < seqlen; i++)
			{
				// fprintf(stderr, "Printing round %d on the LCD display ...\n", attempts);
				if (i == 0)
				{
					sprintf(elem, "Color %d", (i + 1));
					printLCD(lcd, round, elem);
				}
				// For 2 second collect button presses
				...
				while (startT == stopT)
				{
					if (readButton(gpio, pinButton) == HIGH && buttonPressed == LOW)
					{
						// fprintf(stderr, "Press!\n");
						count++;
						buttonPressed = HIGH;
					...
					else if (readButton(gpio, pinButton) == LOW && buttonPressed == HIGH)
						buttonPressed = LOW;
				}

				count = (count < colors) ? count : colors;
				blinkN(gpio, pinRed, 1); // End of element input
				delay(DELAY_LONG);
				blinkN(gpio, pinGreen, count); // Number of button presses
				attSeq[i] = count;			   // Add to sequence
				switch (i) {
					case 0:
						a = count;
						sprintf(elem, "Color %d  %d ", (i + 1), a);
						break;
					case 1:
						b = count;
						sprintf(elem, "Color %d  %d %d", (i + 1), a, b);
						break;
					case 2:
						c = count;
						sprintf(elem, "Color %d  %d %d %d", (i + 1), a, b, c);
						break;
				}
				printLCD(lcd, round, elem);
			}
		...
		// No delays in debug mode to reduce wait time
		if (debug) { delay(DELAY); }
		printf("Finished sequence\n");
		blinkN(gpio, pinRed, 2); // End of sequence input
		int *matches = countMatches(attSeq, theSeq);
		showMatches(matches,theSeq, attSeq, 0); // 0 so we don't print secret sequence in console
		exactNum = matches[0];
		approxNum = matches[1];
		sprintf(exact, "Exact: %d", matches[0]);
		sprintf(approx, "Approx: %d", matches[1]);
		printLCD(lcd, exact, approx);

		blinkN(gpio, pinGreen, matches[0]); // Exact matches
		if (debug) { delay(DELAY); }
		blinkN(gpio, pinRed, 1); // Separator
		if (debug) { delay(DELAY); }
		blinkN(gpio, pinGreen, matches[1]); // Approx matches
	...
	if (found)
	{
		printf("Success! You won the game!\n");
		writeLED(gpio, pinRed, HIGH);
		blinkN(gpio, pinGreen, 3);

		char attemptsStr[17];
		sprintf(attemptsStr, "Attempts: %d", attempts);
		printLCD(lcd, "SUCCESS", attemptsStr);
		if (debug) { delay(4000); }
		writeLED(gpio, pinRed, LOW);
		lcdClear(lcd);
	}
	else
	{
		...
		printLCD(lcd, "Game Over", "Nice Try ;)");
		if (debug) { delay(4000); }
		writeLED(gpio, pinRed, LOW);
		lcdClear(lcd);
	}
